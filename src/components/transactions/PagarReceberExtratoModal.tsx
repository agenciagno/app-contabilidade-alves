...
import { FileText, Table2, Download, Image } from 'lucide-react';
...
interface PagarReceberExtratoModalProps {
  open: boolean;
...
// Extrato de Pagar/Receber — duplicata do relatório de "Conta Corrente" (BankReportModal),
// mas puxando as transações em aberto já carregadas na tela de Pagar/Receber (props),
// em vez de consultar o histórico de movimentações pagas no banco. Saldo Inicial/Final
// usa o saldo real das contas como ponto de partida, igual ao Capital de Giro da tela.
export function PagarReceberExtratoModal({ open, onOpenChange, transactions, banks, categories, contacts, mode = 'all' }: PagarReceberExtratoModalProps) {
  ...
  const dateKeyOf = (t: Transaction): string | null =>
    isReceivables ? (t.due_date || t.expected_date || t.issue_date) : (t.expected_date || t.due_date || t.issue_date);

  // Mesmo pré-filtro da tela de Pagar/Receber: na aba "A Receber", só Honorários Contábeis.
  ...
  const filteredRows = useMemo(() => {
    let result = txnsBase.filter(t => !t.is_paid && dateKeyOf(t));

    if (startDate) result = result.filter(t => (dateKeyOf(t) as string) >= startDate);
    if (endDate) result = result.filter(t => (dateKeyOf(t) as string) <= endDate);
    ...
    if (selectedBankIds.length > 0) result = result.filter(t => t.bank_id && selectedBankIds.includes(t.bank_id));

    result.sort((a, b) => (dateKeyOf(a) || '').localeCompare(dateKeyOf(b) || ''));
    return result;
  }, [txnsBase, startDate, endDate, categoryId, contactId, selectedBankIds, isReceivables]);

  const baseBalance = useMemo(() => {
    const relevant = selectedBankIds.length > 0 ? activeBanks.filter(b => selectedBankIds.includes(b.id)) : activeBanks;
    return relevant.reduce((s, b) => s + Number(b.current_balance), 0);
  }, [activeBanks, selectedBankIds]);

  const rows = useMemo(() => {
    let saldo = baseBalance;
    return filteredRows.map(t => {
      const amt = Number(t.amount);
      saldo += t.type === 'receita' ? amt : -amt;
      return {
        id: t.id,
        date: formatDateBR(dateKeyOf(t) || ''),
        contact_name: getContactLegalName(t.contact) || null,
  ...
  }, [filteredRows, baseBalance, banks]);

  const openingBalance = baseBalance;
  ...
  const eventSummary = (() => {
    ...
    for (const r of rows) {
      ...
      const cur = map.get(name) || { name, qty: 0, entradas: 0, saidas: 0 };
      cur.qty += 1;
  ...
  const accountsLabel = selectedBankIds.length > 0
    ? activeBanks.filter(b => selectedBankIds.includes(b.id)).map(b => b.name).join(', ')
    : 'Todas';
  ...
  const contactLabel = contactId !== 'all'
    ? getContactLegalName(contacts.find(c => c.id === contactId)) || 'Todos'
    : 'Todos';

  const reportTitle = isReceivables ? 'Extrato — A Receber' : 'Extrato — Pagar/Receber';
  const filePrefix = isReceivables ? 'extrato-a-receber' : 'extrato-pagar-receber';
  ...
  const exportPDF = () => {
    ...
    doc.text(`${rows.length} lançamentos • Gerado em ${pad2(today.getDate())}/${pad2(today.getMonth() + 1)}/${today.getFullYear()}`, 14, sepY + 5);

    doc.setTextColor(0);

    autoTable(doc, {
      startY: sepY + 10,
      head: [['Vencimento', 'Banco', 'Cliente/Fornecedor', 'Evento', 'Entrada', 'Saída', 'Saldo']],
      body: rows.map(r => [
        r.date,
        r.bank_name || '—',
        r.contact_name || '—',
      ...
      ...zebraPorData(rows.map(r => r.date)),
      columnStyles: {
    ...
    doc.save(`${filePrefix}-${startDate}-${endDate}.pdf`);
  };

  // ─── XLS Export ───────────────────────────────────────────────────
  const exportXLS = () => {
    const headers = ['Vencimento', 'Banco', 'Histórico', 'Cliente/Fornecedor', 'Evento Contábil', 'Valor Entrada', 'Valor Saída', 'Saldo'];
    ...
    a.href = url;
    a.download = `${filePrefix}-${startDate}-${endDate}.xls`;
    a.click();
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${filePrefix}-${startDate}-${endDate}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };
...
${transactionsOfx}
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${filePrefix}-${startDate}-${endDate}.ofx`;
    a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const exportImage = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `resumo-${filePrefix}-${startDate}-${endDate}.jpg`;
      a.click();
    } catch (err) {
      ...
      <DialogContent className="sm:max-w-2xl">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            >
              <div>
                <h3 className="font-bold text-gray-900 text-sm">{company?.name || reportTitle}</h3>
              ...
              <div className="border-t border-gray-100 pt-1.5">
                <p className="text-[10px] text-gray-400">
                  {rows.length} lançamentos • Gerado em {pad2(today.getDate())}/{pad2(today.getMonth() + 1)}/{today.getFullYear()}
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
              ...
              <Button variant="outline" className="flex items-center gap-1.5 h-8 text-xs" onClick={exportOFX}>
