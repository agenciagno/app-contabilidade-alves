...
export function gerarPdfDiagnostico(d: Diagnostico, nomeEmpresa: string) {
  const doc = new jsPDF();
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(
    `${nomeEmpresa} · ${SETOR_LABEL[d.input.setor]} · ${REGIME_LABEL[d.input.regimeAtual]}${d.atual.anexo ? ` (Anexo ${d.atual.anexo})` : ''}`,
    margem,
    19
  );
  doc.text(
  ...
  if (noUnificado) {
    doc.setTextColor(...MARCA);
    doc.text(`${brl(d.atual.cargaConsumo)} por ano, como hoje`, margem, y);
  } else {
    doc.setTextColor(subiu ? 190 : 20, subiu ? 40 : 130, 40);
    doc.text(`${subiu ? '+' : '-'} ${brl(Math.abs(d.variacaoValor))} por ano`, margem, y);
  }
  y += 9;
  ...
  autoTable(doc, {
    startY: y,
    head: [['Cenário', 'Tributos sobre consumo (ano)', '% do faturamento']],
    body: [
      [
  ...
  autoTable(doc, {
    startY: y,
    head: [['Tributo', 'Valor no ano', 'Substituído pela reforma?']],
    body: d.atual.linhas.map((l) => [l.tributo, brl(l.valor), l.consumo ? 'Sim' : 'Não']),
    theme: 'grid',
    headStyles: { fillColor: MARCA, fontSize: 9 },
  ...
  autoTable(doc, {
    startY: y,
    head: [['Regime', 'Total no ano', 'Alíquota efetiva', 'Situação']],
  ...
  if (d.economiaMigracao > 0 && d.melhorRegime) {
    paragrafo(
      `Migrar para ${d.melhorRegime.label} economizaria cerca de ${brl(d.economiaMigracao)} por ano. Vale confirmar com análise individual antes de mudar.`,
      9,
      MARCA
    );
  }
  ...
  if (d.hibrido) {
    ...
    autoTable(doc, {
      startY: y,
      head: [['Cenário', 'Custo no ano', 'Crédito repassado a clientes PJ']],
      body: [
        [
          'Continuar no DAS unificado',
          brl(d.hibrido.unificadoTotal),
          brl(d.hibrido.creditoRepassadoUnificado),
        ],
        [
          'Regime regular de IBS/CBS',
          brl(d.hibrido.hibridoTotal),
          brl(d.hibrido.creditoRepassadoHibrido),
        ],
      ],
      theme: 'grid',
      headStyles: { fillColor: MARCA, fontSize: 9 },
